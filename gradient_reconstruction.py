...
for i, h in enumerate(h_max):
    ...
    with torch.no_grad():
        ...
        pseudo_pot[h] = grad
    # print(grad.shape)
    v_acc[h] = np.sqrt(np.average((grad - vs[h][:ndata]) ** 2, axis=-1)) / np.sqrt(
        np.average((vs[h][:ndata]) ** 2, axis=-1)
    )
    g_acc.append(
        np.sqrt(np.average((grad - vs[h][:ndata]) ** 2, axis=-1))
        / np.sqrt(np.average((vs[h][:ndata]) ** 2, axis=-1))
    )
#%%
g_acc = [np.average(g) for g in g_acc]
plt.plot(h_max, g_acc)
plt.show()
#%%
h = 2.40
x = ms[h][0:1]
...
vs = [  # np.load('data/unet_dataset/valid_unet_periodic_1024_l_4.5_h_15000_n.npz')['potential'],
    ...
    np.load("data/unet_dataset/valid_unet_periodic_256_l_4.5_h_15000_n.npz")[
        "potential"
    ],
    np.load("data/unet_dataset/valid_unet_periodic_128_l_4.5_h_15000_n.npz")[
        "potential"
    ],
    np.load("data/unet_dataset/valid_unet_periodic_64_l_4.5_h_15000_n.npz")[
        "potential"
    ],
...
models_name = [
    ...
    ["h_4.5_augmentation_ising_model_unet_gelu_256_size_3_layers_30_hc_ks_2_ps"]
    * (n_sample[0]),
    ["h_4.5_augmentation_ising_model_unet_gelu_128_size_3_layers_30_hc_ks_2_ps"]
    * n_sample[1],
    ["h_4.5_augmentation_ising_model_unet_gelu_64_size_3_layers_30_hc_ks_2_ps"]
    * n_sample[2],

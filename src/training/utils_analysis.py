...
def parallel_nambu_diagonalization_ising_model(nbatch,l:int,j_coupling:float,hs:np.array,device:str,pbc:bool):
    ...
    for i in trange(nbatch):
        ...
        s_z_different=pt.einsum('aik,aik->ai',u,pt.conj(u))-pt.einsum('aik,aik->ai',v,pt.conj(v))
...
def nuv_representability_check(model:nn.Module,z:np.ndarray,v:np.ndarray,plot:bool,gs_z:np.ndarray):
